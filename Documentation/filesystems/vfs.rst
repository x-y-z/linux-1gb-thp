...
``dirty_folio``
	called by the VM to mark a folio dirty.  This is particularly
	...
	Filesystems which use buffer_heads may leave this NULL, or
	define their own implementation and call block_dirty_folio() to
	mark the buffer heads as dirty.  Filesystems which use their own
	private data may wish to use filemap_dirty_folio() to ensure
	that the normal pagecache protocols for dirty pages are followed.
	Filesystems which do not write back dirty data can call
	dirty_folio_no_writeback().
