	...
	if (compound_order(head) == HPAGE_PUD_ORDER) {
		if (map_order == HPAGE_PMD_ORDER) {
			...
			if (atomic_add_negative(-1, pmd_compound_mapcount_ptr(page))) {
				...
				__mod_node_page_state(page_pgdat(page), NR_ANON_THPS, -HPAGE_PMD_NR);
			}
		     ...
		     unsigned long address, void *arg)
{
	...
	if (flags & TTU_SPLIT_HUGE_PUD)
		split_huge_pud_address(vma, address, false, page);
	...
	while (page_vma_mapped_walk(&pvmw)) {
		...
		if (pvmw.pte)
			pgtable_pfn = pte_pfn(*pvmw.pte);
		else if (!pvmw.pte && pvmw.pmd)
			pgtable_pfn = pmd_pfn(*pvmw.pmd);
		else if (!pvmw.pte && !pvmw.pmd && pvmw.pud)
			pgtable_pfn = pud_pfn(*pvmw.pud);

		VM_BUG_ON(!pgtable_pfn);
		flush_cache_page(vma, address, pgtable_pfn);
		     ...
		     unsigned long address, void *arg)
{
	 ...
	 * TTU_SPLIT_HUGE_PMD and TTU_SPLIT_HUGE_PUD and it wants to freeze.
	 */
	if (flags & TTU_SPLIT_HUGE_PUD)
		split_huge_pud_address(vma, address, true, page);
...
void try_to_migrate(struct page *page, enum ttu_flags flags)
{
	 ...
	 * TTU_SPLIT_HUGE_PMD and TTU_SPLIT_HUGE_PUD and TTU_SYNC flags.
	 */
	if (WARN_ON_ONCE(flags & ~(TTU_RMAP_LOCKED | TTU_SPLIT_HUGE_PMD |
					TTU_SPLIT_HUGE_PUD | TTU_SYNC)))
		return;
