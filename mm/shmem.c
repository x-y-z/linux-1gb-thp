				   ...
				   struct mm_struct *charge_mm)
{
	...
	VM_BUG_ON(expected && folio_test_multi(folio));
		...
		struct shmem_inode_info *info, pgoff_t index)
{
	...
	folio = vma_alloc_folio(gfp, HPAGE_PMD_ORDER, &pvma, 0, numa_node_id(),
				true);
	shmem_pseudo_vma_destroy(&pvma);
	if (!folio)
		count_vm_event(THP_FILE_FALLBACK);
			...
			struct shmem_inode_info *info, pgoff_t index)
{
	...
	shmem_pseudo_vma_init(&pvma, info, index);
	folio = vma_alloc_folio(gfp, 0, &pvma, 0, numa_node_id(), false);
	shmem_pseudo_vma_destroy(&pvma);
