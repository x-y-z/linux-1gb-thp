...
static inline int calculate_order(unsigned int size)
{
	 ...
	 */
	order = slab_order(size, 1, CONFIG_FORCE_MAX_ZONEORDER, 1);
	if (order < CONFIG_FORCE_MAX_ZONEORDER)
		return order;
	return -ENOSYS;
}
...
static int __init setup_slub_max_order(char *str)
{
	...
	slub_max_order = min(slub_max_order, (unsigned int)CONFIG_FORCE_MAX_ZONEORDER - 1);
