...
again:
		...
		if (is_hugepd(__hugepd(pmd_val(*pmdp))))
			err = walk_hugepd_range((hugepd_t *)pmdp, addr, next, walk, PMD_SHIFT);
		else
			  ...
			  struct mm_walk *walk)
{
 ...
 again:
		...
		if (is_hugepd(__hugepd(pud_val(*pudp))))
			err = walk_hugepd_range((hugepd_t *)pudp, addr, next, walk, PUD_SHIFT);
		else
