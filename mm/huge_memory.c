		...
		unsigned long haddr, bool freeze)
{
	...
	if (freeze) {
		for (i = 0; i < HPAGE_PUD_NR; i += HPAGE_PMD_NR) {
			...
			__mod_lruvec_page_state(page, NR_ANON_MAPPED, -HPAGE_PMD_NR);
			put_page(page + i);
		}
	}
}
...
int promote_list_to_huge_page(struct page *head, struct list_head *list)
{
	...
	mod_node_page_state(page_pgdat(head),
			NR_ISOLATED_ANON + page_is_file_lru(head), -HPAGE_PMD_NR);
out_unlock:
	if (anon_vma) {
		anon_vma_unlock_write(anon_vma);
		put_anon_vma(anon_vma);
...
int promote_list_to_huge_pud_page(struct page *head, struct list_head *list)
{
	...
	list_for_each_entry(subpage, list, lru) {
		...
		struct deferred_split *ds_queue = get_deferred_split_queue(subpage);
	...
	mod_node_page_state(page_pgdat(head),
			NR_ISOLATED_ANON + page_is_file_lru(head), -HPAGE_PUD_NR);
out_unlock:
	if (anon_vma) {
		anon_vma_unlock_write(anon_vma);
		put_anon_vma(anon_vma);
