		  ...
		  struct vm_area_struct *vma)
{
	...
	if (unlikely(page_needs_cow_for_dma(vma, pud_page(pud)))) {
		...
		spin_unlock(src_ptl);
		spin_unlock(dst_ptl);
		__split_huge_pud(vma, src_pud, addr, false, NULL);
		return -EAGAIN;
	}
		...
		unsigned long haddr, bool freeze)
{
	...
	for (i = 0, addr = haddr; i < HPAGE_PUD_NR;
		...
		if (atomic_inc_and_test(pmd_compound_mapcount_ptr(&page[i]))) {
	...
	if (compound_mapcount(page) > 1 && !TestSetPagePUDDoubleMap(page)) {
		...
		/* distinguish between pud compound_mapcount and pmd compound_mapcount */
			atomic_inc(pmd_compound_mapcount_ptr(&page[i]));
	}

	lock_page_memcg(page);
	if (atomic_add_negative(-1, compound_mapcount_ptr(page))) {
		/* Last compound_mapcount is gone. */
		__mod_lruvec_page_state(page, NR_ANON_THPS_PUD,
					-HPAGE_PUD_NR);
		if (TestClearPagePUDDoubleMap(page)) {
			/* No need in mapcount reference anymore */
			for (i = 0; i < HPAGE_PUD_NR; i += HPAGE_PMD_NR)
		/* distinguish between pud compound_mapcount and pmd compound_mapcount */
				atomic_dec(pmd_compound_mapcount_ptr(&page[i]));
		}
	}
	unlock_page_memcg(page);
...
{
	VM_BUG_ON_PAGE(thp_order(page) != HPAGE_PUD_ORDER, page);
...
int total_mapcount(struct page *page)
{
	...
	if (PageHuge(page))
		return compound;
	ret = compound;
	/* if PMD, read all base page, if PUD, read the pmd_compound_mapcount()*/
	if (compound_order(page) == HPAGE_PMD_ORDER) {
		for (i = 0; i < nr; i++)
			ret += atomic_read(&page[i]._mapcount) + 1;
	} else if (compound_order(page) == HPAGE_PUD_ORDER) {
		for (i = 0; i < HPAGE_PUD_NR; i += HPAGE_PMD_NR)
			ret += pmd_compound_mapcount(&page[i]);
		for (i = 0; i < nr; i++)
			ret += atomic_read(&page[i]._mapcount) + 1;
...
int page_trans_huge_mapcount(struct page *page, int *total_mapcount)
{
	...
	if (compound_order(page) == HPAGE_PMD_ORDER) {
		...
		for (i = 0; i < nr; i += HPAGE_PMD_NR) {
			...
			mapcount = pmd_compound_mapcount(&page[i]);
			ret = max(ret, mapcount);
			_total_mapcount += mapcount;
...
{
	VM_BUG_ON_PAGE(thp_order(page) != HPAGE_PMD_ORDER, page);
				     ...
				     unsigned int new_order)
{
	...
	if (PageAnon(head) && new_order && new_order != HPAGE_PMD_ORDER) {
		WARN_ONCE(1, "Split THP to non-zero order or non PMD order not support");
		return -EINVAL;
	}
				...
				unsigned long vaddr_end)
{
	...
	for (addr = vaddr_start; addr < vaddr_end; addr += PAGE_SIZE) {
		...
		if (thp_order(page) == HPAGE_PUD_ORDER && !split_huge_pud_page(page))
			split++;

		if (thp_order(page) == HPAGE_PMD_ORDER && !split_huge_page(page))
			split++;
