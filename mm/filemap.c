 ...
 *    ->private_lock		(__free_pte->block_dirty_page)
 ...
 *    ->private_lock		(zap_pte_range->block_dirty_page)
 *
 * ->i_mmap_rwsem
...
{
	if (folio_test_single(folio) || folio_test_hugetlb(folio))
		return false;
	if (index >= max)
		return false;
...
static struct file *do_sync_mmap_readahead(struct vm_fault *vmf)
{
	...
	if (vmf->vma->vm_flags & VM_HUGEPAGE) {
		...
		/*
		 * Fetch two huge pages, so we get the chance to actually
...
static bool filemap_map_pmd(struct vm_fault *vmf, struct folio *folio)
{
	...
	if (pmd_none(*vmf->pmd) && folio_test_transhuge(folio) &&
			folio_order(folio) == HPAGE_PMD_ORDER) {
		vm_fault_t ret = do_set_pmd(vmf, &folio->page);
		if (!ret) {
			/* Page is mapped successfully, reference consumed. */
			folio_unlock(folio);
			return true;
		}
	}
