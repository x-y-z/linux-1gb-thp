...
retry_isolate1:
	...
	mod_node_page_state(page_pgdat(page1),
			NR_ISOLATED_ANON + page_is_file_lru(page1),
...
retry_isolate2:
	...
	mod_node_page_state(page_pgdat(page2),
			NR_ISOLATED_ANON + page_is_file_lru(page2),
		...
		bool migrate_all)
{
	...
	if (!err)
		mod_node_page_state(page_pgdat(from_page), NR_ISOLATED_ANON +
					page_is_file_lru(from_page), hpage_nr_pages(from_page));
...
set_from_status:
	...
	if (!err)
		mod_node_page_state(page_pgdat(to_page), NR_ISOLATED_ANON +
					page_is_file_lru(to_page), hpage_nr_pages(to_page));
put_and_set_to_page:
	/*
	 * Either remove the duplicate refcount from
	 * isolate_lru_page() or drop the page ref if it was
